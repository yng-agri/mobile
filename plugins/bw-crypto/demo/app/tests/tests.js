...
describe('aesEncrypt', () => {
    it('should successfully encrypt data', async () => {
        ...
        const encValue = await bwCrypto.aesEncrypt(data.buffer, iv.buffer, key.buffer);
    ...
    it('should successfully encrypt and then decrypt data', async () => {
        ...
        const encValue = await bwCrypto.aesEncrypt(data.buffer, iv.buffer, key.buffer);
        const decValue = await bwCrypto.aesDecrypt(encValue, iv.buffer, key.buffer);
...
describe('aesDecrypt', () => {
    it('should successfully decrypt data', async () => {
        ...
        const decValue = await bwCrypto.aesDecrypt(data.buffer, iv.buffer, key.buffer);
...
describe('rsaDecrypt', () => {
    it('should successfully decrypt data', async () => {
        ...
        const decValue = await bwCrypto.rsaDecrypt(data.buffer, privKey.buffer, 'sha1');
        expect(Utils.fromBufferToUtf8(decValue)).toBe('EncryptMe!');
    });
});
...
describe('rsaExtractPublicKey', () => {
    it('should successfully extract key', async () => {
        const privKey = Utils.fromB64ToArray(RsaPrivateKey);
        const publicKey = await bwCrypto.rsaExtractPublicKey(privKey.buffer);
...
function testRsaGenerateKeyPair(length) {
    it('should successfully generate a ' + length + ' bit key pair', async () => {
        const keyPair = await bwCrypto.rsaGenerateKeyPair(length);
        expect(keyPair[0] == null || keyPair[1] == null).toBe(false);
        const publicKey = await bwCrypto.rsaExtractPublicKey(keyPair[1]);
        expect(Utils.fromBufferToB64(keyPair[0])).toBe(Utils.fromBufferToB64(publicKey));
    }, 30000);
}

function makeStaticByteArray(length) {
